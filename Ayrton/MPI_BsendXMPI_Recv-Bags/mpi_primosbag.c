...
int main(int argc, char* argv[]) { /* mpi_primosbag.c  */
  ...
  int cont = 0, total = 0;
  int i, n;
  int meu_ranque, num_procs, inicio, dest, tamanho, raiz = 0, tag = 1, stop = 0;
  void* buffer;
  MPI_Status estado;
  ...
  if (meu_ranque == 0) {
    MPI_Pack_size(num_procs - 1, MPI_INT, MPI_COMM_WORLD, &tamanho);
    tam_buffer = tamanho + MPI_BSEND_OVERHEAD * (num_procs - 1);
  } else {
    MPI_Pack_size(1, MPI_INT, MPI_COMM_WORLD, &tamanho);
    tam_buffer = tamanho + MPI_BSEND_OVERHEAD;
  }
